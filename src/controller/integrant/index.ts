...
import {
  ...
  getGeneralMembers,
  deleteGeneralMember,
} from "../../repository/repository.integrants";
...
export default class IntegrantController extends Controller {
  ...
  public async newLeader(
    @Path("integrantId") integrantId: string
   ...
   */
  @Post("/general")
  public async newIntegrant(
    @Body() information: { user: string; functionsDescription: string },
    @Inject() user: UserI
  ): Promise<IntegrantResponse> {
  ...
  public async deleteGeneralMember(
    @Path("integrantId") integrantId: string
  ): Promise<boolean> {
