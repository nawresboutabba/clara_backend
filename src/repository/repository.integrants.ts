...
export const newIntegrant = async (
  user: UserI,
...
export const deleteGeneralMember = async (
  integrantId: string
): Promise<boolean> => {
  return new Promise(async (resolve, reject) => {
    try {
      const integrant = await IntegrantService.getIntegrantActiveById(
        integrantId
      );
    ...
    try {
      ...
      if (check.exist && check.isActive) {
        if (check.role == COMMITTE_ROLE.GENERAL) {
           ...
           */
          const leader = await IntegrantService.convertToLeader(
            integrantId,
            currentData
          );
...
export const getGeneralMembers = async (): Promise<
  Array<IntegrantResponse>
> => {
  return new Promise(async (resolve, reject) => {
    try {
