...
router.post(
  ...
  async (
    ...
    try {
      ...
      const solutionController = new SolutionController();
      const solution = await solutionController.applyTransition(
        req.params.solutionId,
        req.body,
        req.resources.solution
      );
      res.json(solution).status(200).send();
    } catch (error) {
      next(error);
    }
  }
...
router.patch(
  ...
  [
    ...
    body(
      "is_privated",
      VALIDATIONS_MESSAGE_ERROR.SOLUTION.IS_PRIVATE_INVALID
    ...
    body("WSALevel_chosed", "WSALevel_chosed can not be empty")
      .notEmpty()
      .isIn([WSALEVEL.COMPANY, WSALEVEL.AREA]),
    body("WSALevel_chosed").custom(
      async (value: string, { req }): Promise<void> => {
        try {
          /**
          ...
          if (req.resources.solution.canChooseWSALevel) {
            ...
            if (value == WSALEVEL.AREA) {
              ...
              if (
                areas_available == undefined ||
                areas_available.length === 0
              ) {
                return Promise.reject(
                  "Insert at least an area when WSALevel is AREA"
                );
              }
              const areasAvailable = await AreaService.getAreasById(
                req.body.areas_available
              );
     ...
     */
    body(
      "participation.chosed_mode",
      "participation.mode_chosed invalid"
    ).custom(async (value: string, { req }): Promise<void> => {
      try {
        /**
         ...
         */
        if (
          req.resources.solution.challenge.participationModeAvailable.includes(
            value
          )
        ) {
          return Promise.resolve();
    ...
    body("participation.chosed_mode").custom(
      async (value: string, { req }): Promise<void> => {
        try {
          /**
           ...
           */
          if (
            value == PARTICIPATION_MODE.TEAM &&
            !req.body.participation.team_name
          ) {
          ...
          if (
            ...
            const team = await TeamService.getTeamByName(
              req.body.participation.team_name
            );
            if (team) {
  ...
  async (
    ...
    try {
      ...
      const solutionController = new SolutionController();
      const solution = await solutionController.updateSolution(
        req.params.solutionId,
        req.body,
        req.resources,
        req.user,
