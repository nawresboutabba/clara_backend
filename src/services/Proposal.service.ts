...
import ChallengeProposal, { ChallengeProposalI } from '../models/challenge-proposal';
...
const ChallengeProposalService = {
  async newProposal(proposal: any): Promise<ChallengeProposalI> {
    try{
      const resp: ChallengeProposalI = await ChallengeProposal.create(proposal)
      return resp
    }catch(error){
  ...
  async getChallengeProposal(id: string): Promise<ChallengeProposalI> {
    try{
      const resp:ChallengeProposalI = await ChallengeProposal.findOne({proposalId: id})
      return resp
    }catch(error){
      return Promise.reject(new ServiceError(
  ...
  async listProposals(query: QueryChallengeForm): Promise<any> {
    try{
      ...
      const proposalChallenges = await ChallengeProposal
        .find({...findQuery})
        .skip(query.init)
        .limit(query.offset)
      /**
