...
export async function CAN_VIEW_CHALLENGE(
  req: RequestMiddleware
): Promise<void> {
  try {
    if (!req.resources?.challenge) {
      const challenge: ChallengeI =
        await ChallengeService.getChallengeActiveById(
    ...
    if (req.resources.challenge.WSALevelChosed == WSALEVEL.AREA) {
      const userAreaVisible = user.areaVisible.filter((area) => {
        return area.areaId;
      });
      const challengeAreasAvailable =
        req.resources.challenge.areasAvailable.filter((area) => {
          return area.areaId;
        });
      const intersection = _.intersection(
        challengeAreasAvailable,
        userAreaVisible
      );
      ...
      return Promise.reject(
        new RoutingError(ERRORS.ROUTING.CHALLENGE_FORBIDDEN, HTTP_RESPONSE._500)
      );
    } else {
      /**
