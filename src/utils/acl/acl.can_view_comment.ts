...
export async function CAN_VIEW_COMMENT (req: RequestMiddleware) : Promise<void>{
  try{
     ...
     */
    const comment: CommentI = await CommentService.getComment(req.params.commentId)    
    if (!comment){
    ...
    const solution: SolutionI = await SolutionService.getSolutionActiveById(comment.solution.solutionId)    
