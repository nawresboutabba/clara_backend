...
export async function CAN_VIEW_SOLUTION(req: RequestMiddleware): Promise<void> {
  try {
    ...
    const externalOpinion = solution.externalOpinion.filter(
      (eo) => eo.userId == user.userId
    );
    ...
    if (user.externalUser) {
      ...
      if (req.resources.solution.WSALevelChosed == WSALEVEL.AREA) {
        ...
        const solutionAreasAvailable =
          req.resources.solution.areasAvailable.filter((area) => {
            return area.areaId;
          });
        const intersection = _.intersection(
          solutionAreasAvailable,
          userAreaVisible
        );
