...
export async function IS_VALIDATOR_OF_SOLUTION(
  ...
  try {
    ...
    const integrantsGroupValidator =
      await IntegrantService.getIntegrantsOfGroupValidator(groupValidator);
    const user = integrantsGroupValidator.filter(
      (integrant) => integrant.user.userId == req.user.userId
    );
    if (user.length > 0) {
