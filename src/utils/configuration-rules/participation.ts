...
export const isCompositionUsersValid = async (author: UserI, coauthor: UserI[], configuration: ChallengeI | ConfigurationDefaultI ): Promise<boolean> => {
  ...
  if (!areThereExternalsUser) {
    ...
    /**
      ...
      */
    if (configuration.externalContributionAvailableForCommittee
      ...
      /**
        ...
        */
    } else if (!configuration.externalContributionAvailableForCommittee
      && !configuration.externalContributionAvailableForGenerators) {
      return Promise.resolve(false)
      /**
        * User can invite external users like committee member
        */
    } else if (configuration.externalContributionAvailableForCommittee && committeeMember.isActive) {
      return Promise.resolve(true)
      /**
        * User can invite external users like generator
        */
    } else if (configuration.externalContributionAvailableForGenerators && !committeeMember.isActive) {
      return Promise.resolve(true)
    } else {
      /**
