...
import { genericChallengeFilter } from "../../routes/challenges/challenge.serializer";

export const genericChallengeProposalFilter = async (
  challengeProposal: ChallengeProposalI
): Promise<ChallengeProposalResponse> => {
  try {
    const challengeInformation = await genericChallengeFilter(
      challengeProposal
    );
    const resp = {
...
export const genericArrayChallengeProposalFilter = async (
  challengeProposals: ChallengeProposalI[]
): Promise<ChallengeProposalResponse[]> => {
  try {
